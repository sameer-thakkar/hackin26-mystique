...
- Managed skill directories in this repo are **`<logical-name>-ho/`** (for example `commit-ho/`). The YAML `name:` field stays the logical name **without** `-ho` (e.g. `commit`) so `Skill("commit")` and `/commit` keep working.
- `skill-rules.json` is **not** prefixed.
