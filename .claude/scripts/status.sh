...
# Format: ~/proj 145K 72% | main U:6 | ✓ Last done → Current focus
# Critical: ~/proj ⚠ 160K 80% | main U:6 | Current focus
...
# ─────────────────────────────────────────────────────────────────
# CWD - Short directory name for multi-tab identification
# ─────────────────────────────────────────────────────────────────
# Show parent/current (1 level deep)
cwd_short=$(echo "$cwd" | awk -F'/' '{print $(NF-1)"/"$NF}')
...
# ─────────────────────────────────────────────────────────────────
# Check thoughts/ledgers/ first (new pattern), then root (legacy)
ledger=$(ls -t "$project_dir"/thoughts/ledgers/*.md 2>/dev/null | head -1)
[[ -z "$ledger" ]] && ledger=$(ls -t "$project_dir"/CONTINUITY_CLAUDE-*.md 2>/dev/null | head -1)
...
if [[ "$context_pct" -ge 80 ]]; then
    ...
    output="\033[36m${cwd_short}\033[0m $ctx_display"
