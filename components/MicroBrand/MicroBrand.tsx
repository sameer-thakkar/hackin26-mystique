...
class MicroBrand extends Component<any, any> {
    ...
    render() {
        ...
        const languageProps = {
            ...
            languages,
            languageDropdown:
                CMSContent.data.enable_localization_menu === 'Yes',
        };
        ...
        const dropdownLinksArray = CMSData.dropdown_menu.reduce((acc, item) => {
            if (item.link)
                return [
                    ...acc,
                    { value: item.link.url, label: item.link_text },
                ];
            else return acc;
        }, []);
        const headerProps = {
        ...
        const allTours = CMSData.all_tours.reduce((accum, tour) => {
            ...
            return {
                ...
                [tourData.tgid]: {
                    ...
                    descriptors:
                        tourData.descriptors ||
                        scorpioData[tourData.tgid].descriptors,
                    productHighlights:
                        scorpioData[tourData.tgid].productHighlights,
        ...
        let categories = raw_category.reduce((accum, category) => {
            let cat_id = category.category_name
                .replace(/\s/g, '_')
                .toLowerCase();
