...
export default class Microsite extends Component<any, any> {
        ...
        if (checkIfToursAvailable) {
                ...
                (accum, elem) => {
                    if (elem.tour_variant_id) {
                        return [
                            [
                                ...accum[0],
                                { tgid: elem.tgid, tid: elem.tour_variant_id },
                            ],
                            [...accum[1]],
                        ];
                    } else {
            ...
            const tourGroupPrices = tourGroup.tourGroups.reduce(
                (accum, res, index) => ({
                    ...
                    [tourGroup.tourGroups[index].id]: {
                        price: res.listingPrice
                            ? res.listingPrice.finalPrice
                            : '',
            ...
            const mapVariantPrices = variants.map((tourVariant: any, index) => {
                const inv = tourVariant.inventoryList.find(
                    inventoryList =>
                        inventoryList.tourId == variantTgids[index].tid
                );
                return {
            ...
            const showEarliestAvailability =
                enableEarliestAvailability === 'Yes';
    ...
    render() {
        ...
        const microbrandCardsHeading = this.props.data.data
            .microbrand_cards_heading
        ...
        if (showGroupBooking) {
            uncategorizedToursList
                .filter(function(tour) {
                    return !groupBookingExcludedTgids.find(function(
                        excludedTour
                    ) {
        ...
        return (
            <div>
                <div className="microsite-container">
                    ...
                    {longFormContent ? (
                        <LongForm content={longFormContent} />
                    ) : null}
                    {customFooter ? (
                        <footer>
                        ...
                        <Footer
                            logoUrl={
                                uploadedFooterLogoUrl || footerLogoUrl || null
                            }
                            footerLinks={footerLinks ? footerLinks : null}
                            disclaimer={disclaimer ? disclaimer : null}
                            footerAltText={
                                footerAltText || footerAltTextUploaded || null
                            }
