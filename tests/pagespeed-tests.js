            ...
            if (index === array.length - 1) {
                return {
                    ...
                    perf_score_out_of_100: Math.ceil(
                        (accum.perf_score_out_of_100 +
                            item.perf_score_out_of_100) /
                            array.length
                    ),
                };
            }
...
module.exports = (env, trackInsights = false) => {
    const pagespeedFetchers = domains
        .map(domain =>
            env === 'stage'
                ? domain.replace('https://', 'https://stage.')
                : domain
        )
        .map(domain =>
            ['mobile', 'desktop'].map(
    ...
    return Promise.all(pagespeedFetchers)
        .then(responses => {
            ...
            if (
                aggregateDesktopInsights.perf_score_out_of_100 <
                BENCHMARK_PERF.DESKTOP
            ) {
                signale.fatal(
                    new Error(
            ...
            if (
                aggregateMobileInsights.perf_score_out_of_100 <
                BENCHMARK_PERF.MOBILE
            ) {
                signale.fatal(
                    new Error(
