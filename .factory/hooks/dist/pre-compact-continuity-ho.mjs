...
// src/pre-compact-continuity-ho.ts
import * as fs2 from "fs";
import * as path from "path";

// src/transcript-parser-ho.ts
...
var currentFile = import.meta.url;
var isMainModule = currentFile === `file://${process.argv[1]}` && currentFile.includes("transcript-parser-ho");
if (isMainModule) {
  const args = process.argv.slice(2);
  if (args.length === 0) {
    console.log("Usage: npx tsx transcript-parser-ho.ts <transcript-path> [session-name]");
    process.exit(1);
  }
...
async function main() {
  ...
  let ledgerFiles = [];
  try {
    ledgerFiles = fs2.readdirSync(ledgerDir).filter((f) => f.endsWith(".md") && !f.startsWith("."));
  } catch {
  }
  ...
  if (input.trigger === "auto") {
    ...
    if (transcriptPath && transcriptPath.trim() && fs2.existsSync(transcriptPath)) {
      const summary = parseTranscript(transcriptPath);
    ...
    if (!handoffCreated) {
      const fallbackHandoff = generateFallbackHandoff(projectDir, sessionName, input.session_id);
      if (fallbackHandoff) {
        const handoffDir = path.join(projectDir, "thoughts", "shared", "handoffs", sessionName);
        fs2.mkdirSync(handoffDir, { recursive: true });
        const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").slice(0, 19);
        handoffFile = `auto-handoff-fallback-${timestamp}.md`;
        const handoffPath = path.join(handoffDir, handoffFile);
        fs2.writeFileSync(handoffPath, fallbackHandoff);
        handoffCreated = true;
      }
    }
    ...
    const message = handoffCreated ? `[PreCompact:auto] Created ${handoffFile} in thoughts/shared/handoffs/${sessionName}/` : `[PreCompact:auto] Session summary auto-appended to ${mostRecent}`;
    const output = {
      continue: true,
...
function generateFallbackHandoff(projectDir, sessionName, sessionId) {
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const lines = [];
  ...
  if (editedFiles.length === 0 && gitChanges.length === 0 && buildAttempts.passed === 0 && buildAttempts.failed === 0) {
    return null;
  }
  lines.push("---");
  lines.push(`date: ${timestamp}`);
  lines.push("type: auto-handoff");
  lines.push("trigger: pre-compact-fallback");
  lines.push(`session: ${sessionName}`);
  lines.push("note: Created from cache files (transcript_path was empty)");
  lines.push("---");
  lines.push("");
  lines.push("# Auto-Handoff (PreCompact Fallback)");
  lines.push("");
  lines.push("This handoff was automatically generated before context compaction.");
  lines.push("Note: Full transcript was not available, using cache files as fallback.");
  lines.push("");
  if (editedFiles.length > 0) {
    lines.push("## Files Modified");
    lines.push("");
    editedFiles.forEach((f) => lines.push(`- ${f}`));
    lines.push("");
  }
  if (gitChanges.length > 0) {
    ...
    lines.push("");
    lines.push("```");
    gitChanges.forEach((c) => lines.push(c));
    lines.push("```");
    lines.push("");
  }
  if (buildAttempts.passed > 0 || buildAttempts.failed > 0) {
    ...
    lines.push("");
    lines.push(`- Passed: ${buildAttempts.passed}`);
    lines.push(`- Failed: ${buildAttempts.failed}`);
    lines.push("");
  }
  lines.push("## Suggested Next Steps");
  lines.push("");
  lines.push("1. Check the continuity ledger for current task state");
  lines.push("2. Review modified files to understand recent changes");
  lines.push("3. Run `git status` to see uncommitted work");
  lines.push("4. Check the TaskList for pending items");
  lines.push("");
  return lines.join("\n");
}
